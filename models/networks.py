...
class Dis_pair(nn.Module):
    ...
    def _make_net(self, ch, input_dim, n_layer, norm, sn):
        model = []
        model = model + [LeakyReLUConv2d(input_dim, ch, kernel_size=3, stride=2, padding=1, norm=norm, sn=sn)]
        tch = ch
        for i in range(1, n_layer):
            model = model + [LeakyReLUConv2d(tch, tch * 2, kernel_size=3, stride=2, padding=1, norm=norm, sn=sn)]
            tch = tch * 2
        model = model + [LeakyReLUConv2d(tch, tch * 2, kernel_size=3, stride=2, padding=1, norm='None', sn=sn)]
        tch = tch * 2
        if sn:
            model = model + [nn.utils.spectral_norm(nn.Conv2d(tch, 1, kernel_size=1, stride=1, padding=0))]
        else:
            model = model + [nn.Conv2d(tch, 1, kernel_size=1, stride=1, padding=0)]
        return nn.Sequential(*model)

    def forward(self, image_a, image_b):
        # print('image_a的类型', image_a.type())     torch.cuda.FloatTensor
        # print('image_b的类型', image_b.type())     torch.cuda.FloatTensor
        out = torch.cat((image_a, image_b), 1)
        # print('out1的类型', out.type())            torch.cuda.FloatTensor
        model = self.model.cuda()
        out = model(out)
        # print('out2的类型', out.type())
        out = out.view(-1)
        outs = []
        outs.append(out)
        return outs
...
class Dis_content(nn.Module):
    def __init__(self, ndf=256):
        ...
        model = []
        model = model + [LeakyReLUConv2d(ndf, ndf, kernel_size=7, stride=2, padding=1, norm='Instance')]
        model = model + [LeakyReLUConv2d(ndf, ndf, kernel_size=7, stride=2, padding=1, norm='Instance')]
        model = model + [LeakyReLUConv2d(ndf, ndf, kernel_size=7, stride=2, padding=1, norm='Instance')]
        model = model + [LeakyReLUConv2d(ndf, ndf, kernel_size=4, stride=1, padding=0)]
        model = model + [nn.Conv2d(ndf, 1, kernel_size=1, stride=1, padding=0)]
...
class MultiScaleDis(nn.Module):
    ...
    def _make_net(self, ch, input_dim, n_layer, norm, sn):
        ...
        for _ in range(1, n_layer):
            ...
            tch = tch * 2
        if sn:
            model = model + [nn.utils.spectral_norm(nn.Conv2d(tch, 1, 1, 1, 0))]
        else:
...
class LeakyReLUConv2d(nn.Module):
    def __init__(self, inplanes, outplanes, kernel_size, stride, padding=0, norm='None', sn=False):
        ...
        if sn:
            model = model + [nn.utils.spectral_norm(nn.Conv2d(inplanes, outplanes, kernel_size=kernel_size, stride=stride, padding=0, bias=True))]
        else:
            model = model + [nn.Conv2d(inplanes, outplanes, kernel_size=kernel_size, stride=stride, padding=0, bias=True)]
        if norm == 'Instance':
            model = model + [nn.InstanceNorm2d(outplanes, affine=False)]
        model = model + [nn.LeakyReLU(inplace=True)]
        self.model = nn.Sequential(*model)
    def forward(self, x):
        return self.model(x)
